...
func TestIndex_SearchPrefix(t *testing.T) {
	...
	results := idx.Search(SearchOptions{Query: "con", Tab: model.TabAll})
	if len(results) != 2 {
...
func TestIndex_SearchSubstring(t *testing.T) {
	...
	results := idx.Search(SearchOptions{Query: "odel", Tab: model.TabAll})
	if len(results) != 1 {
...
func TestIndex_SearchSubsequence(t *testing.T) {
	...
	results := idx.Search(SearchOptions{Query: "mgo", Tab: model.TabAll})
	if len(results) != 1 {
		t.Fatalf("expected 1 result, got %d", len(results))
	}
...
func TestIndex_ExtFilter(t *testing.T) {
	...
	results := idx.Search(SearchOptions{Query: "", Tab: model.TabAll, ExtFilter: ".go"})
	if len(results) != 2 {
...
func TestIndex_NoMatch(t *testing.T) {
	...
	results := idx.Search(SearchOptions{Query: "zzzzz", Tab: model.TabAll})
	if len(results) != 0 {
...
func TestIndex_MaxResults(t *testing.T) {
	...
	results := idx.Search(SearchOptions{Query: "file", Tab: model.TabAll, MaxResults: 10})
	if len(results) != 10 {
...
func TestIndex_ConcurrentReads(t *testing.T) {
	...
	for range 10 {
		...
		go func() {
			defer wg.Done()
			for range 100 {
				results := idx.Search(SearchOptions{Query: "f", Tab: model.TabAll})
				if len(results) == 0 {
